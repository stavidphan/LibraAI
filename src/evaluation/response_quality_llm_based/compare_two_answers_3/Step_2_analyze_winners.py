...
def analyze_winners(data, output_dir, top_k):
    ...
    winners_count = {category: {"Answer 1": 0, "Answer 2": 0, "Equal": 0} for category in categories}
    total_requests = len(data)
    failed_parses = 0
    valid_requests = 0
    fixed_jsons = []  # Store fixed JSONs
    invalid_jsons = []  # Store JSONs that could not be fixed
    quality_metrics = {f'topk{top_k}': {category: [0.0, 0.0, 0.0] for category in categories}}
    ...
    for request in data:
        try:
            ...
            for category in categories:
                ...
                if winner in ["Answer 1", "Answer 2", "Equal"]:
    ...
    if valid_requests > 0:
        for category in categories:
            ...
            equal_count = winners_count[category]["Equal"]
            total_counts = answer1_count + answer2_count + equal_count
            if total_counts > 0:
                answer1_percent = round((answer1_count / total_counts) * 100, 2)
                answer2_percent = round((answer2_count / total_counts) * 100, 2)
                equal_percent = round((equal_count / total_counts) * 100, 2)

                quality_metrics[f'topk{top_k}'][category] = [answer1_percent, answer2_percent, equal_percent]
            else:
                answer1_percent = answer2_percent = equal_percent = 0.0
            ...
            summary_lines.append(f"  Equal: {equal_count} times ({equal_percent:.2f}%)\n")
    else:
...
for top_k in topk_arr:
    file_path = f"./data/eval5/compare_3_with_old/batch_eval_topk6_output.jsonl"
    output_dir = f"./data/eval5/compare_3_with_old"
